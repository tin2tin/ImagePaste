...
class PasteImageToVideoSequenceEditor(Operator):
    """Paste image(s) from clipboard as image strips"""

    bl_idname = "impaste.paste_as_video"
    bl_label = "Paste Images From Clipboard"
    bl_options = {"UNDO_GROUPED"}
...
def menu_func_as_video(self, context):
    self.layout.separator()
    self.layout.operator(PasteImageToVideoSequenceEditor.bl_idname, icon="IMAGE_PLANE")
...
classes = (
    ...
    PasteImageToVideoSequenceEditor,
...
def register():
    ...
    bpy.types.SEQUENCER_MT_context_menu.append(menu_func_as_video)
    ...
    if kc:
        ...
        kmi = km.keymap_items.new(
            PasteImageToVideoSequenceEditor.bl_idname,
            type="V",
            value="PRESS",
            ctrl=True,
            shift=True,
